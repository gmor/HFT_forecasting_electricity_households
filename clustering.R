...
GaussianMixtureModel_clustering<-function(df,time_column="t",value_column="v",k=NULL,title="",tz="UTC",plot_file=NULL, latex_font=F){
    ...
    if(latex_font==T){
        ...
        print(ggplot(df_structural)+
                ...
                theme_bw()+theme(legend.position="none",axis.text=element_text(size=16),
                                 axis.title=element_text(size=16))+labs(x="Hour of the day", y="Hourly consumption (kWh)") +
                theme(text= element_text(size=20, family="CM Roman")))
